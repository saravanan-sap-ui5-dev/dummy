...
public class MiscellaneousDAO {
    ...
    public List<Miscellaneous> getApplications(boolean isAllRecords, Long id, MiscellaneousEntity miscellaneousEntity, Integer pageNumber, Integer pageSize) {
        try {
            String filterParams = "";
            if (miscellaneousEntity != null) {
                filterParams = new Gson().toJson(miscellaneousEntity);
            }
            miscellaneousEntity = miscellaneousEntity == null ? new MiscellaneousEntity()
                    : miscellaneousEntity;

            StoredProcedureQuery sp_GetPropertyGenerator = entityManager
                    .createStoredProcedureQuery("GET_MISCELLANEOUS", "Miscellaneous_Mapping");
            ...
            sp_GetPropertyGenerator.setParameter("PAGE_NUMBER", miscellaneousEntity.getPageNumber());
            sp_GetPropertyGenerator.setParameter("PAGE_SIZE", miscellaneousEntity.getPageSize());
            sp_GetPropertyGenerator.setParameter("SORTING_KEY", miscellaneousEntity.getSortingKey());
            sp_GetPropertyGenerator.setParameter("ORDER_BY", miscellaneousEntity.getOrderBy());
            sp_GetPropertyGenerator.setParameter("STRING_TYPE", miscellaneousEntity.getStringType());

            sp_GetPropertyGenerator.execute();
            @SuppressWarnings("unchecked")
            List<Miscellaneous> results = sp_GetPropertyGenerator.getResultList();
            return results;
    ...
    public Collection< Miscellaneous>addEditApplication(
        ...
        try {
            ...
            String reqPayLoad = gson.toJson(miscellaneous);

            StoredProcedureQuery sp_AddEditPropertyGenerator = entityManager
                    .createStoredProcedureQuery("ADD_EDIT_MISCELLANEOUS", "Miscellaneous_Mapping");
            ...
            List<Miscellaneous> results = sp_AddEditPropertyGenerator.getResultList();
            return results;
