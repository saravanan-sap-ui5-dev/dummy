sap.ui.define([
    ...
    "use strict";
    var that = this;
    return BaseController.extend("com.lighthouse.master_data.hs_code.AddEditHSCode", {
        ...
        onInit: function () {
            ...
            this.oRouter.getRoute("hsCodeDetailCreate").attachMatched(this._onRouteCreateMatched, this);
            this.oRouter.getRoute("hsCodeDetailEdit").attachMatched(this._onRouteEditMatched, this);

            let getSource = (id => this.getView().byId(id));
            [this.btnEdit, this.formId, this.pageId, this.popoverBtn] = [getSource("btnEdit"), getSource("hsForm"), getSource("page_addEditHsCode"), getSource("btnHSCodeErr")];
        ...
        initialValues: function () {
            ...
            this._moduleId = URLConstants.moduleId.hs_code;
            this.setModel();
        ...
        navAfterCreation: function () {
            ...
            if (sPreviousHash == "hsCodeDetailCreate") {
                navData.route = "hsCodeMaster";
            } else {
                navData.route = null;
            }
        ...
        fetchHsCodeById: async function () {
            try {
                this.showLoading(true);
                let oModel = this.getView().getModel();
                let path = URLConstants.URL.hs_code_by_id.replace("{id}", this._item);
                let hsCode = await this.restMethodGet(path);
                if (hsCode) {
                    hsCode.title = hsCode.name + ' (' + hsCode.code + ')';
                    hsCode.statusText = this.masterData.status.find(ele => hsCode.status === ele.value)?.description;
                    oModel.getData().hsCode = hsCode;
                    oModel.refresh(true);
                }
        ...
        postHsCode: async function () {
            try {
                var that = this;
                let postMdl = this.getView().getModel();
                let postData = postMdl.getData().hsCode;
                ...
                if (valid.length == 0) {
                    ...
                    if (this._item) {
                        ...
                        this.getRouter().navTo("hsCodeDetailEdit", {
                            id: postRes.id
                        });
                    }
                    this.showMessage();
        ...
        onPressCancel: function () {
            this.errorPopoverParams();
            if (this._route !== "hsCodeDetailCreate") {
                this.fetchHsCodeById();
