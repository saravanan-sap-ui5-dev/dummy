sap.ui.define([
    ...
    "use strict";
    var that = this;
    return BaseController.extend("com.lighthouse.master_data.cargo_type.CargoType", {
        ...
        onInit: function () {
            ...
            this.oRouter.getRoute("cargoType").attachMatched(this._onRouteMatched, this);

            this._tableId = this.byId("table_CargoType");
            this._pageId = this.byId("page_CargoType");
            this._filterBar = this.byId("ca_filterBar");
            this.popoverBtn = this.byId("btnManageCargoTypeErr");
        ...
        emptyModelData: function () {
            return {
                ...
                appliedFilter:{   /*Filtering on Pagination*/
                    id: null, 
                    name: null,
                    code: null,
                    cargoCodeCollection: null,
        ...
        initFunctionalities: function () {
            ...
            let isRelatedHash = prevHash.includes("cargoTypeDetailCreate") || prevHash.includes("cargoTypeDetailEdit");
            if (!isRelatedHash) {
            ...
            if (sDirection.includes("Backwards") || sortingKey) { //Get Direction to change the model data
                ...
                if (exFilter) {
                    ...
                    fData.cargoCodeCollection = null;
                }
                oModel.refresh(true);
        ...
        onAfterRendering: function () {
            ...
            dynamicPageElement.getElementsByClassName("sapFDynamicPageContentWrapper")[0].addEventListener("scroll", function (event) {
                ...
                if (element.scrollHeight - element.scrollTop - element.clientHeight < 1) {
                    ...
                    // let count = oModel.getData().advancedFilter.pageNumber > noOfFetch;
                    if (!count) {
        ...
        onPressCargoTypeCreate: function (oEvent) {
            let oSettingMdl = this.getOwnerComponent().getModel("settings");
            oSettingMdl.getData().visible_filter = this.getFilterVisibleFields();
            this.getRouter().navTo("cargoTypeDetailCreate");
        },
        ...
        onPressBulkUpload: async function (oEvent) {
            ...
            if (this.excelUploadDialog) {
                this.excelUploadDialog.setTitle("Upload Cargo Type");
            }
        },
        ...
        bulkPostData: async function (data) {
            let cargoTypeData = data.multiple_worksheet["Cargo Type"];
            cargoTypeData.forEach(e => {
        ...
        bulkPost: async function (data) {
            ...
            if (selFileName) {
                let valid = await this.onExcelValidation(this.jsonData?.multiple_worksheet["Cargo Type"]);
                if (valid && selFileName) {
                    ...
                    try {
                        ...
                        MessageBox.success(msg, {
                            title: that.getResourceProperty("crtUploadedSuccessfully"),
        ...
        sheetDetails: function () {
            let obj = {
                fileName: "Cargo Type.xlsx",
                sheets: {
                    sheetName: ["Cargo Type"]
                },
                columns: [
                    {
                        label: 'ID',
        ...
        onPressTemplate: async function () {
            ...
            let file = await fetch("./master_data/cargo_type/CargoType_template.xlsx")
                .then((res) => res.url)
                .then((file) => {
                    window.open(file);
