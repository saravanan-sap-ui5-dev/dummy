    ...
    return {
        ...
        loadIds: function (isCreate, tableId, noDataId, formId, eMdl, pageId) {
            if (isCreate) {
                ...
                this._hsCode_noData = noDataId;
            }
            else {
                this._hsCode_formId = formId;
                this.eMdl = eMdl;
                this.pageId = pageId;
            }
        },
        onAfterRendering: function (oModel) {
            ...
            if (!count) {
                ...
                oModel.refresh(true);
                console.log("Reached the bottom of the scroll HS Code.");
                return true;
            }
            else{
                return false;
        ...
        fetchHsCode: async function (oModel) {
            try {
                ...
                let hsCodeRes = await BaseController.prototype.restMethodPost(path, request);
                if (hsCodeRes.length > 0) {
                    hsCodeRes.map(e => {
                ...
                if (exItems.length > 0) {
                    oModel.getData().masterScreenTable.items = [...exItems, ...hsCodeRes];
                } else {
                    oModel.getData().masterScreenTable.items = hsCodeRes;
                }
        ...
        postHsCode: async function (oModel) {
